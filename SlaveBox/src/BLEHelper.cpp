...
void BLEHelper::begin(const String &deviceName, const String &boxId) {
    ...
    pimpl->pSensorCharacteristic->addDescriptor(new BLE2902());
    ...
    // Make sensor-type characteristic readable and notifiable so the client can
    // either read it or receive a notification immediately before a value
    // notification. This matches typical client expectations.
    pimpl->pSensorTypeCharacteristic = pService->createCharacteristic(
        SENSOR_TYPE_CHAR_UUID,
        BLECharacteristic::PROPERTY_READ | BLECharacteristic::PROPERTY_NOTIFY
    );
    pimpl->pSensorTypeCharacteristic->addDescriptor(new BLE2902());
